...
class BaseBars(ABC):
    ...
    def _assert_csv(test_batch: pd.DataFrame):
        ...
        """
        assert test_batch.shape[1] == 4, 'Must have only 4 columns in csv: date_time, price, volume & amount.'
        ...
        assert isinstance(test_batch.iloc[0, 3], float), 'amount column in csv not float.'
    ...
    def _get_imbalance(self, price: float, signed_tick: int, volume: float, amount: float) -> float:
        """
...
class BaseImbalanceBars(BaseBars):
    ...
    def _extract_bars(self, data: Tuple[dict, pd.DataFrame]) -> list:
        ...
        list_bars = []
        for row in data:
            # if self.tick_num % 100000 == 0:
            #     print(f"scan {self.tick_num} ticks, created {len(list_bars)} bars")
            # Set variables
            date_time = row[0]
            self.tick_num += 1
            ...
            imbalance = self._get_imbalance(price, signed_tick, volume, dollar_value)
            ...
            if np.abs(self.thresholds['cum_theta']) > self.thresholds['exp_num_ticks'] * np.abs(
                ...
                # print(f"new bar: cum_theta {self.thresholds['cum_theta']}, exp_num_ticks {self.thresholds['exp_num_ticks']} "
                #       f"* expected_imbalance {self.thresholds['expected_imbalance']}")
